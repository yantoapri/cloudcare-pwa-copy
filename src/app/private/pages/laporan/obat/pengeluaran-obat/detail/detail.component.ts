...
export class DetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe((params: Params) => {
      if (params) {
        this.obat=params.obat
        this.params.date_from=params.tgl1
        this.params.date_to=params.tgl2
        this.periode=moment(new Date(parseInt(params.tgl1))).format("DD/MM/YYYY")+" - "+moment(new Date(parseInt(params.tgl2))).format("DD/MM/YYYY")
  ...
  download(resp){
		...
		link.href = url;
		link.setAttribute('download', `Detail Laporan Pengeluaran Obat.xlsx`);
  ...
  showDataTables() {
    ...
    return {
      ...
      ajax : (dataTablesParameters: any, callback: any) => {
        ...
        this.pengeluaran.getObatDetail(dataTablesParameters)
        .subscribe((resp : any) => {
          callback({
