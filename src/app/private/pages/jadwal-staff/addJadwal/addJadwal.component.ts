...
@Component({
	selector: 'app-add',
	templateUrl: './addJadwal.component.html',
	// styleUrls: ['./add.component.sass']
})
export class addJadwal implements OnInit {
	...
	prosesSelectStaff(event: any, aksi: string) {
		...
		if (aksi == 'search')
		{
			...
			this.listStaff=[]
			param.last_data=0
			this.isLastStaf=false
			}else{
			this.isLastStaf=true
			}
		}
					...
					if(resp.response.length==0){
						...
						if(resp.response.length<10){
							...
							resp.response.map(val=>{
								let i=this.listStaff.findIndex(x=>x.id_akun_staff==val.id_akun_staff)
								if(i==-1)
